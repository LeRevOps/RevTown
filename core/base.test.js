...
// ── Helpers ───────────────────────────────────────────────────────────────────
function mockSearchReturns(records) {
    mockSearch.mockImplementation(async (_token, _type, _filters, _props, onBatch) => {
        if (records.length > 0)
            await onBatch(records, records.length);
        return { total: records.length };
    });
}
function mockClaudeReturns(text) {
